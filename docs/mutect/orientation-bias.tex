...
Next in library prep we add adapters to the double-stranded inserts. In ICE Kapa Exome protocol, we only use one kind of double-stranded adapter, shown in red and green in Figure \ref{fig:oxog}. What this means is that the clones of the same double-stranded adapter attach to both sides of the insert (they are flipped due to the asymmetry of DNA). Note that the ends of adapters are not base-paired; that is, the sequences on the two strands are not complementary. This will play a crucial role later on. \todo{Must be more specific here} A \textit{library} refers to an insert with adapters attached on both ends. We will subsequently make many copies of each \textit{original library} via PCR.
...
In the E step, we compute the responsibilities $\gamma_{nk} = p(z_{nk} | x_n, m_n, a)$. $\gamma_{nk}$ is the posterior probability of $k$th component of $z$ given the observed variables $x_n$ and $m_n$ and the observed allele $a$. By Bayes rule we have

\begin{align}
\gamma^*_{nk} &\propto p(z_{k} | x_n, m_n, a) \nonumber \\
		        &\propto p(z_{k} | a) p(m_n | z_k, f_{k} ) p(x_n | m_n ) \nonumber \\
...
\E [ \ln p(X, M , Z | \vpi, \vp, \vtheta, \va) ] &= \E [ \ln \prod_{n = 1}^{N}  p(z_n | a_n) p(m_n | z_n, p_{z} ) p(x_n | m_n, z_n, \theta_n ) ] \nonumber \\
							&= \E [ \ln \prod_{n = 1}^{N} \{ \prod_{k = 1}^{K} \pi_{ak}^{z_{kn}} \} \{ \prod_{k = 1}^{K} ( f_k^{m_n} (1-f_k)^{R_n - m_n} ) ^{z_{nk}} \} \{ \prod_{k = 1}^{K} ( \theta_k^{x_n} (1 - \theta_k)^{m_n - x_n} )^{z_{nk}} \} ] \notag \\
...
$N_k$ is the effective number of samples that were in state $z_k$ and $N_{ak}$ the effective number of samples in state $z_k$ \textit{ and } have allele $a$. $\bar{x}_k$ and $\bar{m}_k$ are the effective sample means of $x_k$ and $m_k$, and $\bar{R}_k$ is the effective mean depth over the sites in state $z_k$. We now take the derivative of (\ref{eq:expjointLL}) with respect to $\theta_k, p_k, \pi_{ak}$, set them equal to 0, and solve for those parameters to get the update equations.
