...
if [[ "$#" -lt 7 ]]; then
    ...
    echo -e "  [4] maximum time of idling (in min) before cluster self-termination begins (required)"
    echo -e "  [5] GCS directory containing the correct reference & indices (required)"
    echo -e "  [6] GCS directory containing the BAM and its index (required), and"
    echo -e "either when a local initialization script is to be uploaded to a bucket:"
    echo -e "  [7] local initialization script"
    echo -e "  [8] GCS path to upload initialization script to (required as per Google)"
    echo -e "or when the initialization script is already in the cloud: "
    echo -e "  [7] path to initialization script on GCS"
